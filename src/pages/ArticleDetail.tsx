...
export default function ArticleDetail() {
  ...
  const bodyText = article.body || article.description || `${article.title}. This article covers the latest developments in cryptocurrency markets and blockchain technology.`;
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_340px] gap-8">
          <article>
            <div className="rounded-xl overflow-hidden mb-6">
              <img src={featureImage} alt={article.title} className="w-full h-64 sm:h-80 lg:h-[420px] object-cover" />
            </div>
            ...
            <div className="flex items-center gap-4 text-sm text-muted-foreground mb-6 pb-6 border-b">
              <span className="flex items-center gap-1.5"><User className="w-4 h-4" />{article.author || article.source}</span>
              <span className="flex items-center gap-1.5"><Clock className="w-4 h-4" />{timeAgo(article.published_at)}</span>
            </div>
            <div className="flex justify-center my-6"><AdBanner width={728} height={90} /></div>
            <div className="prose prose-sm max-w-none text-foreground leading-relaxed whitespace-pre-line">{bodyText}</div>
            <div className="flex justify-center my-8"><AdBanner width={728} height={90} /></div>
          </article>
          ...
          <aside className="space-y-6">
            ...
            <div className="bg-card rounded-xl border p-5">
              ...
              <div className="space-y-3">
                {trending.map((a) => (
                  <Link key={a.id} to={`/article/${a.id}`} className="flex items-start gap-3 group">
                    <img src={a.image_url || "https://images.unsplash.com/photo-1639762681485-074b7f938ba0?w=200&q=80"} alt={a.title} className="w-16 h-12 rounded object-cover shrink-0" />
                    <div className="min-w-0">
                      <h4 className="text-xs font-medium leading-snug line-clamp-2 group-hover:text-primary transition-colors">{a.title}</h4>
